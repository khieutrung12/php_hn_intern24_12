    ...
    <div class="container grid lg:grid-cols-4 gap-6 pt-4 pb-16 items-start relative m-auto">
        ...
        <div
            ...
            <div class="divide-gray-200 divide-y space-y-5 relative">
                <!-- brand filter -->
                <div class="pt-4 mx-3">
                    <h3 class="text-xl text-gray-800 mb-5 uppercase font-medium">{{ __('titles.Brands') }}</h3>
                    <div class="space-y-4">
                        @foreach ($brands as $brand)
                            <div class="flex items-center">
                                <input type="checkbox" id="Dominik"
                                    class="text-indigo-900 focus:ring-0 rounded-sm cursor-pointer">
                                <label for="Dominik" class="text-gray-600 ml-3 cursor-pointer hover:text-indigo-900">
                                    {{ $brand->name }}
                                </label>
                ...
                <!-- price filter -->
                <div class="pt-4 mx-3">
                    <h3 class="text-xl text-gray-800 mb-5 uppercase font-medium">{{ __('titles.Price') }}</h3>
        ...
        <div class="col-span-3">
            ...
            <div class="mb-4 flex items-center justify-end">
                ...
                <select
                    ...
                    @foreach (config('sortproduct') as $key => $sort)
                        <option value="{{ $key }}">{{ __('titles.' . $sort) }}</option>
                    @endforeach
                </select>
            </div>
            <!-- sorting end -->
            ...
            <div class="grid lg:grid-cols-2 xl:grid-cols-3 sm:grid-cols-2 gap-6">
                @foreach ($products as $product)
                    <div class="button-product rounded bg-white shadow overflow-hidden">
                        <div class="relative">
                            <img src="{{ asset('images/uploads/products/' . $product->image_thumbnail)}}" class="w-96 h-96">
                            <div
                        ...
                        <div class="pt-4 pb-3 px-4">
                            <a href="{{ route('show', $product->id) }}">
                                <h4 class="uppercase font-medium text-xl mb-2 text-gray-800 hover:text-indigo-900 transition">
                                    {{ $product->name }}
                                </h4>
                            </a>
                        ...
                        <a href="#"
                            class="block w-full py-1 text-center text-white bg-indigo-900 border border-indigo-900 rounded-b hover:bg-transparent hover:text-indigo-900 transition">
